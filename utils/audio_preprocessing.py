...
def extract_acoustic_features(
    ...
    features = np.vstack([
        ...
        np.mean(log_mel, axis=0, keepdims=True),   # mean across mels per frame
        np.std(log_mel, axis=0, keepdims=True),    # std across mels per frame
    ])  

    return features.T  # (frames, features) for each cycle
